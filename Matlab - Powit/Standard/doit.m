...
% This reduces number of calculations for powit prob and eliminates need to
% retain dependent variable.

XC=zeros(1,NCSMAX,NV,NP); % Explanatory variables with random coefficients 
%                                  for chosen choice situations, for each person
X=zeros(NALTMAX-1,NCSMAX,NV,NP); % Explanatory variables with random coefficients 
%                                  for unchosen choice situations, for each person 
XFC=zeros(1,NCSMAX,NF,NP); % Explanatory variables with fixed coefficients 
%                                  for chosen choice situations, for each person
XF=zeros(NALTMAX-1,NCSMAX,NF,NP); % Explanatory variables with fixed coefficients 
%                                  for unchosen choice situations, for each person
XAC=ones(1,NCSMAX,NA,NP); % Beta variable with fixed coefficient 
%                                  for chosen choice situations, for each person
XA=ones(NALTMAX-1,NCSMAX,NA,NP); % Beta variable with fixed coefficient 
%                                  for unchosen choice situations, for each person
S=zeros(NALTMAX-1,NCSMAX,NP); % Identification of the alternatives in each choice situation, for each person
SC=zeros(1,NCSMAX,NP); % Identification of the chosen alternatives in each choice situation, for each person
...
for n=1:NP;  %loop over people
 ...
 for t=t1:t2; %loop over choice situations
     ...
     SC(1,1+t-t1,n)=ones(1,1);
     if NV>0
        XC(1,1+t-t1,:,n)=xx(cs==t & yy == 1,:);
        X(1:k,1+t-t1,:,n)=xx(cs==t & yy == 0,:);
     end
     if NF>0
        XFC(1,1+t-t1,:,n)=xxf(cs==t & yy == 1,:);
        XF(1:k,1+t-t1,:,n)=xxf(cs==t & yy == 0,:);
     end;
 end
end
...
if NV>0 & NF>0
   param=[F;B(IDV(:,2)~=5,1);W;A];
elseif NV>0 & NF==0
   param=[B(IDV(:,2)~=5,1);W;A];
elseif NV==0 & NF>0;
   param=[F;A];
else
