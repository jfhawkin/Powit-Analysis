...
B_TIME_CAR = Beta('B_TIME_CAR',0,-10,10,0,'Travel time')
B_TIME_TRAIN = Beta('B_TIME_TRAIN',0,-10,10,0,'Travel time')
B_TIME_SM = Beta('B_TIME_SM',0,-10,10,0,'Travel time')
B_COST = Beta('B_COST',0,-10,10,0,'Travel cost')
B_HE = Beta('B_HE',0,-10,10,0,'Vehicle Headway')
...
TRAIN_HE_SCALED = DefineVariable('TRAIN_HE_SCALED', TRAIN_HE / 100)
SM_TT_SCALED = DefineVariable('SM_TT_SCALED', SM_TT / 100.0)
SM_COST_SCALED = DefineVariable('SM_COST_SCALED', SM_COST / 100)
SM_HE_SCALED = DefineVariable('SM_HE_SCALED', SM_HE / 100)
CAR_TT_SCALED = DefineVariable('CAR_TT_SCALED', CAR_TT / 100)
CAR_CO_SCALED = DefineVariable('CAR_CO_SCALED', CAR_CO / 100)



V1 = ASC_TRAIN + B_TIME_TRAIN * TRAIN_TT_SCALED + B_COST * TRAIN_COST_SCALED + B_HE * TRAIN_HE_SCALED
V2 = ASC_SM + B_TIME_SM * SM_TT_SCALED + B_COST * SM_COST_SCALED +B_HE * SM_HE_SCALED
...
exclude = (( PURPOSE != 1 ) * (  PURPOSE   !=  3  ) + ( CHOICE == 0 ) + (CHOICE==2)*(CAR_AV_SP==0)) > 0
