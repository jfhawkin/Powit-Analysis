...
if WANTWGT == 0
    ll=-sum(log(p),2); % Log likelihood across all situations summed by person. 
                       % Minimization so take negative to minimize rather than maximize the log likehilhood (-inf to 0)
    g=-sum(g,2); % Gradient across all situations summed by person
else
    ll=-sum(WGT.*log(p),2);
